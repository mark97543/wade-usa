...
import Greeting from '../openmodal/modal.jsx';


function Home() {
    const [message, setMessage] = useState('');
    // ... (message, countdown state and useEffect)
    const [showModal, setShowModal] = useState(true);


    const handleCloseModal = () => { // Define the handler *before* using it in JSX
      setShowModal(false);
  };
    ...
    //console.log('showModal:', showModal); // Should log `true` on initial render
    return (
      <div>
          <Header />
          ...
          <h2 className="fancy">The Future Mr. & Mrs. Wade</h2>
            ...
            <Greeting show={showModal} onClose={handleCloseModal} />
          <Footer />
      </div>
    );
  }
